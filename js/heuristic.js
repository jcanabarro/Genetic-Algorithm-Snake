function Heuristic (snake, weights) {
    this.calculateFitness = function () {
        ...
        let heuristicValue = (this.weights[ FOOD_DIST_INDEX ] - this.snake.eatCount * 0.01) * this.foodDist(food)
            + this.weights[ SQUARENESS_INDEX ] * this.squareness()
            + this.weights[ COMPACTNESS_INDEX ] * this.compactness()
            + this.weights[ CONNECTIVITY_INDEX ] * this.connectivity()
            + this.weights[ DEAD_END_INDEX ] * this.deadend();
    ...
    this.initGrid = function () {
        ...
        for (let i = 0; i < this.snake.tail.length; i++) {
            if (this.snake.tail[ i ].x / scale !== -1 && this.snake.tail[ i ].x / scale !== cols &&
                this.snake.tail[ i ].y / scale !== -1 && this.snake.tail[ i ].y / scale !== rows) {
                this.grid[ this.snake.tail[ i ].y / scale ][ this.snake.tail[ i ].x / scale ] = 1;
            }
        }
        if (this.snake.x / scale !== -1 && this.snake.x / scale !== cols && this.snake.y / scale !== -1 &&
            this.snake.y / scale !== rows) {
    ...
    this.compactness = function () {
        ...
        for (let i = 0; i < this.snake.tail.length; i++) {
            for (let j = 0; j < this.snake.tail.length; j++) {
                if (this.snake.tail[ i ].x + scale === this.snake.tail[ j ].x &&
                    this.snake.tail[ i ].y === this.snake.tail[ j ].y) {
                    count++;
                } else if (this.snake.tail[ i ].x - scale === this.snake.tail[ j ].x &&
                    this.snake.tail[ i ].y === this.snake.tail[ j ].y) {
                    count++;
                } else if (this.snake.tail[ i ].x === this.snake.tail[ j ].x &&
                    this.snake.tail[ i ].y + scale === this.snake.tail[ j ].y) {
                    count++;
                } else if (this.snake.tail[ i ].x === this.snake.tail[ j ].x &&
                    this.snake.tail[ i ].y - scale === this.snake.tail[ j ].y) {
                    count++;
    ...
    const FOOD_DIST_INDEX = 0;

    const SQUARENESS_INDEX = 1;

    const COMPACTNESS_INDEX = 2;

    const CONNECTIVITY_INDEX = 3;

    const DEAD_END_INDEX = 4;

    this.weights[ FOOD_DIST_INDEX ] = weights[ FOOD_DIST_INDEX ];

    this.weights[ SQUARENESS_INDEX ] = weights[ SQUARENESS_INDEX ];

    this.weights[ COMPACTNESS_INDEX ] = weights[ COMPACTNESS_INDEX ];

    this.weights[ CONNECTIVITY_INDEX ] = weights[ CONNECTIVITY_INDEX ];

    this.weights[ DEAD_END_INDEX ] = weights[ DEAD_END_INDEX ];
}
